const enMessages = {
  hello: 'Hello, world!',
  UserInfoButtonText: 'My Information',
  SaveProfileButtonText: 'Save profile',
  ProfileSaveSuccess: 'Profile saved',
  ResetPasswordButtonText: 'Password Reset',
  LogoutButtonText: 'Logout',
  NoticeTitle: 'Notice',
  ConfirmLogoutText: 'Are you sure you want to logout?',
  CancelButtonText: 'Cancel',
  ConfirmButtonText: 'Confirm',
  TenantProfileSubmitLabel: 'Confirm submit',
  ChangePasswordTitle: 'Password Reset',
  OldPasswordLabel: 'Old Password',
  NewPasswordLabel: 'New Password',
  ConfirmPasswordLabel: 'Confirm Password',
  ConfirmPasswordNotMatch: 'The new password and the confirm password do not match',
  OldPasswordPlaceholder: 'Please enter the old password',
  NewPasswordPlaceholder: 'Please enter the new password',
  ConfirmPasswordPlaceholder: 'Please enter the confirm password',
  ResetPasswordSuccess: 'Password reset successfully',
  ResetPasswordFailed: 'Password reset failed',
  AdminResetPasswordLabel: 'Reset password',
  AdminResetPasswordTitle: 'Reset user password',
  AdminResetPasswordPrompt: 'Enter a new password',
  AdminResetPasswordSuccess: 'Password has been reset',
  LogoutFailed: 'Logout failed',
  FormValidationFailed: 'Form validation failed',
  UserPhotoLabel: 'User Photo',
  UsernameLabel: 'Username',
  TelephoneLabel: 'Telephone',
  EmailLabel: 'Email',
  PlaceLabel: 'Province and City',
  RemarksLabel: 'Remarks',
  AddressLabel: 'Detailed Address',
  GenderLabel: 'Gender',
  RoleLabel: 'Role',
  StatusLabel: 'Status',
  CreatedAtLabel: 'Created At',
  Boy: 'Male',
  Girl: 'Female',
  Active: 'Active',
  Disabled: 'Disabled',
  PasswordLengthError: 'The password must be between 6 and 20 characters',
  AccessDeniedNotice: 'You are not authorized to access',
  UnauthorizedNotice: 'Login status has expired, please log in again',
  RequestFailedNotice: 'Request failed, please contact the administrator',
  RequestTimeoutNotice: 'Request timeout, please check the network or try again later',
  ResourceNotFoundNotice: 'The requested resource does not exist',
  IDLabel: 'ID',
  PhoneLabel: 'Phone',
  UpdatedAtLabel: 'Updated At',
  OperationsLabel: 'Operations',
  DownloadLabel: 'Download',
  PreviewLabel: 'View',
  DetailLabel: 'Detail',
  EditLabel: 'Edit',
  DeleteLabel: 'Remove',
  QueryLabel: 'Query',
  ResetLabel: 'Reset',
  NewButtonLabel: 'New',
  RefreshButtonLabel: 'Refresh',
  ImportButtonLabel: 'Import',
  ImportDialogTitle: 'Import',
  ImportAppCodeLabel: 'Application',
  ImportDryRunLabel: 'Dry-run only',
  ImportFileLabel: 'File',
  ImportFilePlaceholder: 'Select a JSON file',
  ImportSubmitLabel: 'Import',
  ImportSuccessNotice: 'Import succeeded',
  ImportFailedNotice: 'Import failed',
  ImportResultCreated: 'Created',
  ImportResultUpdated: 'Updated',
  ImportResultReactivated: 'Reactivated',
  DeleteConfirmLabel: 'Are you sure to delete the record?',
  DeleteConfirmTitle: 'Delete Confirm',
  DetailTitle: 'Detail',
  CreateTitle: 'New',
  EditTitle: 'Edit',
  RemoveTitle: 'Remove',
  PasswordLabel: 'Password',
  UsernameRequired: 'Please enter username',
  UsernameLengthError: 'Username must be between 3 and 20 characters',
  PasswordRequired: 'Please enter password',
  EmailRequired: 'Please enter email address',
  EmailFormatError: 'Please enter a valid email format',
  PhoneRequired: 'Please enter phone number',
  PhoneFormatError: 'Please enter a valid phone number format (11 digits, starting with 1)',
  RoleRequired: 'Please select a role',
  StatusRequired: 'Please select a status',
  SuperAdmin: 'Super Admin',
  Admin: 'Admin',
  User: 'User',
  Guest: 'Guest',
  Inactive: 'Inactive',
  Locked: 'Locked',
  MethodLabel: 'Method',
  PathLabel: 'Path',
  PermLabelLabel: 'Permission Label',
  ModuleKeyLabel: 'Module Key',
  AppCodeLabel: 'Application',
  AppCodePlaceholder: 'OAuth client id',
  AppCodeRequired: 'Please select an application',
  AppsListLoading: 'Loading applications…',
  AppsPageTitle: 'Applications',
  AppsPageSubtitle: 'Choose an application to enter.',
  AppsPageEnter: 'Enter',
  AppsPageEmpty: 'No applications available',
  AppsPageOpenFailed: 'Unable to open application',
  MethodRequired: 'Please select a method',
  PathRequired: 'Please enter path',
  PermLabelRequired: 'Please enter permission label',
  ModuleKeyRequired: 'Please enter module key',
  RemarkLabel: 'Remark',
  EnableLabel: 'Enable',
  EnabledLabel: 'Enabled',
  DisableLabel: 'Disable',
  Enabled: 'Enabled',
  Enable: 'Enable',
  Disable: 'Disable',
  EnableRequired: 'Please select enable status',
  NameLabel: 'Name',
  NameRequired: 'Please enter name',
  SelectTitle: 'Select',
  ParentNameLabel: 'Parent Name',
  HiddenLabel: 'Hidden',
  Yes: 'Yes',
  No: 'No',
  ClientIdLabel: 'Client ID',
  ClientIdRequired: 'Please enter client ID',
  ResourceIdsLabel: 'Resource IDs',
  ResourceIdsPlaceholder: 'Please enter resource IDs, separated by commas',
  ScopeLabel: 'Scope',
  ScopePlaceholder: 'Please enter scope, separated by commas',
  AuthorizedGrantTypesLabel: 'Authorized Grant Types',
  AuthorizedGrantTypesPlaceholder: 'Please enter authorized grant types, separated by commas',
  WebServerRedirectUriLabel: 'Redirect URI',
  AuthoritiesLabel: 'Authorities',
  AuthoritiesPlaceholder: 'Please enter authorities, separated by commas',
  AccessTokenValidityLabel: 'Access Token Validity',
  RefreshTokenValidityLabel: 'Refresh Token Validity',
  AdditionalInformationLabel: 'Additional Information',
  AdditionalInformationPlaceholder: 'Please enter additional information (JSON format)',
  AutoapproveLabel: 'Auto Approve',
  CodeLabel: 'Code',
  CodeRequired: 'Please enter code',
  ParentOrganizationLabel: 'Parent Organization',
  TypeLabel: 'Type',
  TypeRequired: 'Please select type',
  OrgTypeCompany: 'Company',
  OrgTypeDepartment: 'Department',
  OrgTypeTeam: 'Team',
  ContactNameLabel: 'Contact Name',
  ContactPhoneLabel: 'Contact Phone',
  ContactEmailLabel: 'Contact Email',
  DomainLabel: 'Domain',
  LogoLabel: 'Logo',
  ExpireTimeLabel: 'Expire Time',
  SystemNameLabel: 'System Name',
  AvatarLabel: 'Avatar',
  TenantInitializeButtonLabel: 'Initialize',
  TenantInitializeTitle: 'Initialize tenant',
  TenantInitializeSuccess: 'Initialized successfully',
  TenantInitOrgSectionTitle: 'Organization',
  TenantInitUserSectionTitle: 'User',
  TenantInitInfoTitle: 'Initialization',
  InitializedLabel: 'Initialized',
  UploadTip: 'Only one image file can be uploaded!',
  UploadImageOnly: 'Only image files can be uploaded!',
  UploadImageSizeLimit: 'Image size cannot exceed 2MB!',
  UploadImageError: 'Failed to read image file, please try again',
  SelectFileButtonLabel: 'Select File',
  OriginalNameLabel: 'Original Name',
  FileTypeLabel: 'File Type',
  FileTypeImage: 'Image',
  FileTypeDocument: 'Document',
  FileTypeVideo: 'Video',
  FileTypeAudio: 'Audio',
  FileTypeOther: 'Other',
  FilePathLabel: 'File Path',
  FileSizeLabel: 'File Size',
  MimeTypeLabel: 'MIME Type',
  UrlLabel: 'URL',
  TenantIdLabel: 'Tenant ID',
  TenantNameLabel: 'Tenant name',
  OriginalNameRequired: 'Please enter original name',
  FileTypeRequired: 'Please select file type',
  FilePathRequired: 'Please enter file path',
  TenantNotAvailable: 'The tenant is not available, please contact the administrator',
  WelcomeLogin: 'Login',
  WelcomeRegister: 'Register',
  WelcomeRegisterTodo: 'Registration is not available in this app.',
  WelcomeAllRightsReserved: 'All rights reserved.',
  WelcomePoweredBy: 'Powered by',
  WelcomeEmail: 'Email:',
  WelcomePhone: 'Phone:',
  WelcomeAddress: 'Address:',
  OrganizationLabel: 'Organization',
  OrganizationRequired: 'Please select an organization',
  APIsLabel: 'APIs',
  MenusLabel: 'Menus',
  DataDomainsLabel: 'Data domains',
  IconLabel: 'Icon',
  IconTip: 'Please copy the icon name from the address and paste it into the input box above.',
  MenuLabel: 'Menu',
  ButtonLabel: 'Button',
  PermLabel: 'Permission Label',
  PermRequired: 'Please enter permission label',
  SortLabel: 'Sort',
  SortRequired: 'Please enter sort',
  CloseButtonText: 'Close',
  LoginIpLabel: 'Login IP',
  HttpMethodLabel: 'HTTP method',
  HttpStatusLabel: 'HTTP status',
  RequestUriLabel: 'Request URI',
  DurationMsLabel: 'Duration (ms)',
  UserIdLabel: 'User ID',
  LoginStatusLabel: 'Login Status',
  LoginTimeLabel: 'Login Time',
  LoginLocationLabel: 'Login Location',
  UserAgentLabel: 'User Agent',
  Success: 'Success',
  Failure: 'Failure',
  LoginStatusRequired: 'Please select login status',
  CreateSuccess: 'Create successfully',
  CreateFailed: 'Create failed',
  UpdateSuccess: 'Update successfully',
  UpdateFailed: 'Update failed',
  PageNotFound: 'Page not found',
  BackToHome: 'Back to home',
  WelcomeBackLabel: 'Welcome back',
  SystemSettingTitle: 'System Setting',
  DataDomainManagementTitle: 'Data domains',
  ScopeValueLabel: 'Scope value',
  CustomSqlLabel: 'Custom SQL',
  DescriptionLabel: 'Description',
  PlatformConfigKeyLabel: 'Key',
  PlatformConfigValueLabel: 'Value',
  PlatformConfigTypeLabel: 'Type',
  PlatformConfigDescriptionLabel: 'Description',

  PlatformInitTitle: 'Initialize Platform',
  PlatformInitSubtitle: 'Complete the required information to start using the system.',
  PlatformInitStepTenant: 'Default Tenant',
  PlatformInitStepAdmin: 'Super Admin Account',
  NextLabel: 'Next',
  BackLabel: 'Back',
  SubmitLabel: 'Submit',
  PlatformInitSuccess: 'Initialized successfully',
  SystemNameRequired: 'Please enter system name',
  DomainRequired: 'Please enter domain',
  LogoRequired: 'Please upload logo',
  DomainFormatError: 'Please enter a valid domain or IP',
  DomainPortNotAllowed: 'Port is not allowed in tenant domain',
  PlatformInitDomainPlaceholder: 'Domain or IP (e.g., example.com or 192.168.1.10)',
}

export default enMessages