...
Skylark 是一套**前后端分离**的多租户管理系统，支持快速开发业务的框架。目前包含 **API 网关**、**权限子系统 Permission（OAuth2 + RBAC + Data Domains）**、**业务子系统（示例 + 可脚手架扩展）**。

- **网关**：`gateway`（Spring Cloud Gateway）
- **权限服务**：`permission`（Spring Boot，OAuth2 授权服务器 + RBAC 资源服务 + Data Domains）
- **管理端前端**：`permission-pc`（Docker 运行服务名：`permission-app`，Nginx 托管 + 反代）
- **业务服务示例**：`service/business-service`（Docker 运行服务名：`business-service`）
- **业务前端示例**：`web/apps/business-web`（Docker 运行服务名：`business-web`）
- **数据库**：`mysql`（MySQL 8）
...
- **业务端前端（business-web）**：`http://localhost:9531`（当前 Compose 固定映射）
