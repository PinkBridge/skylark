...
/**
 * Fetches and caches service-to-service OAuth2 access tokens (client_credentials).
 */
@Slf4j
...
public class ServiceTokenProvider {
  ...
  private Token fetchToken() {
    ...
    if (!resp.getStatusCode().is2xxSuccessful() || resp.getBody() == null) {
      throw new IllegalStateException("Failed to fetch service token from permission: " + resp.getStatusCode());
    }
    Map body = resp.getBody();
    Object token = body.get("access_token");
    Object expiresIn = body.get("expires_in");
    if (!(token instanceof String)) {
      throw new IllegalStateException("permission token response missing access_token");
    }
    long seconds = 300L;
    ...
    log.debug("Fetched service token, expires in {}s", seconds);
