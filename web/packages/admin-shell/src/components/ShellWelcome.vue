    ...
    <div class="index-content">
      <div class="logo-section">
        <div class="logo-placeholder">
          <img
            v-if="tenant && tenant.logo && !logoErrored"
            :src="tenant.logo"
            alt="Logo"
            class="tenant-logo"
            @error="onLogoError"
        ...
        <h1 class="app-title">{{ tenant.systemName || tenant.name }}</h1>
        <el-tag v-if="!isTenantAvailable" type="danger">{{ t('TenantNotAvailable') }}</el-tag>
      </div>
      ...
      <div class="footer-section">
        <p>© {{ year }} {{ tenant.name }}. {{ t('WelcomeAllRightsReserved') }}</p>
...
const isTenantAvailable = computed(() => {
  if (!tenant.value || tenant.value.status !== 'ACTIVE') return false
  const expireTimeMs = parseExpireTime(tenant.value.expireTime)
  if (expireTimeMs === null) return true
  return expireTimeMs > Date.now()
})

const handleLogin = () => {
  if (!shell?.getAuthorizationUrl) return
  window.location.href = shell.getAuthorizationUrl()
}

const handleRegister = () => {
  if (typeof shell.onRegister === 'function') {
    shell.onRegister()
    return
  }
  ElMessage.info(t('WelcomeRegisterTodo'))
}

onMounted(async () => {
  if (shell && typeof shell.getTenantForCurrentLocation === 'function') {
    const tenantData = await shell.getTenantForCurrentLocation()
    if (tenantData) {
